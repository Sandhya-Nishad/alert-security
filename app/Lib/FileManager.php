...
class FileManager
{
    ...
    */
    protected function uploadImage(){
        // Try to process image with Intervention Image. If the required PHP extension/driver
        // is not available (e.g. GD), fall back to moving the uploaded file as-is.
        try {
            // prefer imagick if available, otherwise gd
            $driver = extension_loaded('imagick') ? 'imagick' : 'gd';
            // ImageManager constructor expects a driver string or DriverInterface instance
            $manager = new ImageManager($driver);

            // Use make() which accepts uploaded file instance as well
            $image = $manager->make($this->file->getRealPath() ?: $this->file);

            // resize the image if size provided
            if ($this->size) {
            ...
            if ($this->thumb) {
                ...
                $manager->make($this->file->getRealPath() ?: $this->file)->resize($thumb[0], $thumb[1])->save($this->path . '/thumb_' . $this->filename);
            }

        } catch (\Exception $e) {
            // If image driver is unavailable or any error occurs, fallback to simple file move
            // This ensures upload still works even without GD/Imagick installed.
            $this->uploadFile();

            // create a thumbnail copy if requested (no resizing)
            if ($this->thumb) {
