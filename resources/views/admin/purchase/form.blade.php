        ...
        <div class="col-lg-12 col-md-12 mb-30">
            <div class="card">
                ...
                <option value="{{ $warehouse->id }}" 
                        data-zone="{{ $warehouse->zone }}" 
                        @selected($warehouse->id == @$purchase->warehouse_id)>
                    {{ __($warehouse->name) }}
                    @if($warehouse->zone)
                        @php
                            $zones = \App\Http\Controllers\Admin\WarehouseController::getZones();
                        @endphp
                        - ({{ $zones[$warehouse->zone] ?? $warehouse->zone }})
                    @endif
                </option>
            @endforeach
                                    </select>
